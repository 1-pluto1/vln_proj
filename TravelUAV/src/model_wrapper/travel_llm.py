...
#         for i in range(len(episodes)):
#             # 处理图像和指令
#             image = episodes[i][-1]['rgb'][0]  # 获取最新的RGB图像
#             instruction = f"Navigate to the {episodes[i][0].get('target_object', 'target')}"
            
#             # 如果有辅助信息，添加到指令中
#             if assist_notices is not None and assist_notices[i]:
#                 instruction += f" {assist_notices[i]}"
                
#             # 计算相对目标位置
#             rot_to_target = self._calculate_rot_to_target(episodes[i], target_positions[i])
...
#     def _calculate_rot_to_target(self, episode, target_position):
#         # 计算相对目标位置的旋转矩阵
#         # 这部分逻辑可以从TravelModelWrapper中复用
#         # ...
#         return None  # 替换为实际计算结果
...
#             # 使用Fast-in-Slow模型的异步推理模式
#             if hasattr(self.model, 'predict_action'):
#                 # 完整模式
#                 output = self.model.predict_action(
#                     image_head_fast=inputs[i]['image'],
#                     instruction=inputs[i]['instruction'],
#                     predict_mode='diff'  # 或者根据配置选择'diff+ar'
#                 )
#             else:
#                 # 仅使用快速系统
...
#             # 将模型输出转换为航点
#             waypoint = self._convert_output_to_waypoint(output, episodes[i], rot_to_targets[i])
#             waypoints.append(waypoint)
        
#         return np.array(waypoints)
    
#     def _convert_output_to_waypoint(self, output, episode, rot_to_target):
#         # 将模型输出转换为航点格式
#         # 这里需要根据Fast-in-Slow模型的输出格式进行适配
#         # ...
#         return output  # 替换为实际转换结果
    
#     def predict_done(self, episodes, object_infos):
#         # 使用Fast-in-Slow模型预测任务是否完成
#         # 可以使用模型的输出或者其他启发式方法
#         prediction_dones = []
#         for i in range(len(episodes)):
#             # 简单示例：检查是否接近目标
#             current_pos = np.array(episodes[i][-1]['sensors']['imu']['position'])
#             target_pos = np.array(object_infos[i]['position'])
#             distance = np.linalg.norm(current_pos - target_pos)
#             prediction_done = distance < 1.0  # 阈值可调整
#             prediction_dones.append(prediction_done)
