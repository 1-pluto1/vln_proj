...
from vla.datasets import EpisodicRLDSDataset, RLDSBatchTransform, RLDSDataset, UAVBatchTransform, UAVDataset
...
def get_vla_dataset_and_collator(
    ...
    use_uav_dataset: bool = True,            # Flag to use UAV dataset instead of RLDS
    ...
    # Choose batch transform based on dataset type
    if use_uav_dataset:
        batch_transform = UAVBatchTransform(
            action_tokenizer, tokenizer, image_transform, prompt_builder_fn, 
            predict_stop_token=predict_stop_token, load_pointcloud=load_pointcloud,
            action_chunk=action_chunk, lang_subgoals_exist=lang_subgoals_exist,
        )
    else:
