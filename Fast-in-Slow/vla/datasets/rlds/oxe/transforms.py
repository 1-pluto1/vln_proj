...
def uav_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    """
    为UAVDataset设计的标准化函数，用于生成proprio键
    
    Args:
        trajectory: 原始轨迹数据
        
    Returns:
        标准化后的轨迹数据，包含proprio键
    """
    # 检查必要的键是否存在
    ...
    # 从原始数据中提取位置和姿态信息
    if "base_pose_xyz" in trajectory["observation"] and "base_pose_rpy" in trajectory["observation"]:
        # 如果原始数据中已经有分开的位置和姿态信息，直接连接它们
        trajectory["observation"]["proprio"] = tf.concat(
            (
                trajectory["observation"]["base_pose_xyz"],
                trajectory["observation"]["base_pose_rpy"],
            ),
            axis=-1,
        )
    elif "position" in trajectory and "orientation" in trajectory:
        # 如果原始数据中有position和orientation，连接它们
        trajectory["observation"]["proprio"] = tf.concat(
            (
                trajectory["position"],
    ...
    elif "state" in trajectory["observation"]:
        # 如果原始数据中有state键，直接使用它
        if "position" in trajectory["observation"]["state"] and "rotation" in trajectory["observation"]["state"]:
            position = trajectory["observation"]["state"]["position"]
            rotation = trajectory["observation"]["state"]["rotation"]
            trajectory["observation"]["proprio"] = tf.concat((position, rotation), axis=-1)
        else:
            # 如果state是一个完整的向量，直接使用
            trajectory["observation"]["proprio"] = trajectory["observation"]["state"]
    else:
        # 如果没有找到相关信息，创建一个默认的6维向量（3维位置+3维姿态）
        trajectory["observation"]["proprio"] = tf.zeros((trajectory["action"].shape[0], 6), dtype=tf.float32)
    
    # 确保action是正确的格式
    if "action" not in trajectory:
        # 如果没有action，创建一个默认的6维向量
        trajectory["action"] = tf.zeros((trajectory["observation"]["proprio"].shape[0], 6), dtype=tf.float32)
    
    # 添加其他必要的键
    if "task" not in trajectory:
        trajectory["task"] = {}
    
    if "language_instruction" not in trajectory["task"] and "instruction" in trajectory:
        trajectory["task"]["language_instruction"] = trajectory["instruction"]
