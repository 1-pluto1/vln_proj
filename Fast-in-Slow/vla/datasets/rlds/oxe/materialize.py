...
def make_oxe_dataset_kwargs(
    ...
    if dataset_kwargs["action_encoding"] not in [ActionEncoding.EEF_POS_BI, ActionEncoding.EEF_POS, ActionEncoding.EEF_R6, ActionEncoding.UAV_POS]:
        raise ValueError(f"Cannot load `{dataset_name}`; only EEF_POS, EEF_R6 & UAV_POS actions supported!")
    ...
    if dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS:
        dataset_kwargs["absolute_action_mask"] = ([False] * 6 + [True]) * action_chunk
        dataset_kwargs["absolute_proprio_mask"] = [False] * 6 + [True]
        dataset_kwargs["action_normalization_mask"] = ([True] * 6 + [False]) * action_chunk
        dataset_kwargs["proprio_normalization_mask"] = [True] * 6 + [False]
    elif dataset_kwargs["action_encoding"] is ActionEncoding.EEF_POS_BI:
        dataset_kwargs["absolute_action_mask"] = ([False] * 6 + [True] + [False] * 6 + [True]) * action_chunk
        dataset_kwargs["absolute_proprio_mask"] = [False] * 6 + [True] + [False] * 6 + [True]
        dataset_kwargs["action_normalization_mask"] = ([True] * 6 + [False] + [True] * 6 + [False]) * action_chunk
        ...
        dataset_kwargs["absolute_action_mask"] = ([False] * 9 + [True]) * action_chunk
    ...
    elif dataset_kwargs["action_encoding"] is ActionEncoding.UAV_POS:
        # UAV 6-DoF状态 (XYZ + Roll-Pitch-Yaw)，没有夹持器
        dataset_kwargs["absolute_action_mask"] = ([False] * 6) * action_chunk
        dataset_kwargs["absolute_proprio_mask"] = [False] * 6
        dataset_kwargs["action_normalization_mask"] = ([True] * 6) * action_chunk
        dataset_kwargs["proprio_normalization_mask"] = [True] * 6
