...
def normalize_action_and_proprio(traj: Dict, metadata: Dict, normalization_type: NormalizationType):
    ...
    if normalization_type == NormalizationType.NORMAL:
        ...
        for key, traj_key in keys_to_normalize.items():
            ...
            #     match=lambda k, _: k == traj_key,
            #     map_fn=lambda x: (
            #         print(f"Before: {x}") or  # 打印处理前的 x
            #         time.sleep(10) or  # 暂停 10 秒
            #         tf.concat([x[..., :-7], tf.zeros_like(x[..., -7:])], axis=-1)
            #         if tf.reduce_all(tf.equal(x[..., -7:], x[..., -7:][..., 0:1])) else x
            #         or print(f"After: {x}")  # 打印处理后的 x
            #     )
            # )
...
def get_dataset_statistics(
    ...
    # num_transitions, num_trajectories = 0, 0
    # lists_action = [[] for _ in range(14)]
    # lists_proprio = [[] for _ in range(14)]
    # for traj in tqdm(dataset.iterator(), total=cardinality if cardinality != tf.data.UNKNOWN_CARDINALITY else None):
    #     if np.all(traj["action"][:, -7:] == 0):
    #         for sub_traj in traj["action"]:
    #             for i in range(7):
    #                 lists_action[i].append(sub_traj[i])
    #         for sub_traj in traj["proprio"]:
    #             for i in range(7):
    #                 lists_proprio[i].append(sub_traj[i])
    #     else:
    #         for sub_traj in traj["action"]:
    #             for i in range(len(sub_traj)):
    #                 lists_action[i].append(sub_traj[i])
    #         for sub_traj in traj["proprio"]:
    #             for i in range(len(sub_traj)):
    #                 lists_proprio[i].append(sub_traj[i])
    ...
    # # print([len(lst) for lst in lists_action])
    # # print([len(lst) for lst in lists_proprio])

    # stats_actions = [
    #     {
    #         "mean": np.mean(lst).tolist(),
    #         "std": np.std(lst).tolist(),
    #         "max": np.max(lst).tolist(),
    #         "min": np.min(lst).tolist(),
    #         "q01": np.quantile(lst, 0.01).tolist(),
    #         "q99": np.quantile(lst, 0.99).tolist(),
    #     }
    #     for lst in lists_action
    # ]
    # combined_stats_action = {
    #     "mean": [stat["mean"] for stat in stats_actions],
    #     "std": [stat["std"] for stat in stats_actions],
    #     "max": [stat["max"] for stat in stats_actions],
    #     "min": [stat["min"] for stat in stats_actions],
    #     "q01": [stat["q01"] for stat in stats_actions],
    #     "q99": [stat["q99"] for stat in stats_actions],
    # }
    ...
    #         "q99": np.quantile(lst, 0.99).tolist(),
    #     }
    #     for lst in lists_proprio
    # ]
    # combined_stats_proprio = {
    #     "mean": [stat["mean"] for stat in stats_proprio],
    #     "std": [stat["std"] for stat in stats_proprio],
    #     "max": [stat["max"] for stat in stats_proprio],
    #     "min": [stat["min"] for stat in stats_proprio],
    #     "q01": [stat["q01"] for stat in stats_proprio],
    #     "q99": [stat["q99"] for stat in stats_proprio],
    # }

    # metadata = {
    #     "action": combined_stats_action,
    #     "proprio": combined_stats_proprio,
    #     "num_transitions": num_transitions,
    #     "num_trajectories": num_trajectories,
    # }
