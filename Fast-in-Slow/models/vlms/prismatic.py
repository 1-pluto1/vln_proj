...
class PrismaticVLM(VLM):
    def __init__(
        ...
        **kwargs,
        ...
        # allow separate proprio dimension for UAV state
        proprio_dim = kwargs.get("proprio_dim", None)
        if self.load_state:
            self.proprio_embedder = ActionEmbedder(action_size=proprio_dim if proprio_dim is not None else action_dim, hidden_size=token_size)
