...
from models.vlms.pointcloud_processor.lift3d_dino import Lift3dDinov2
...
class FiSvla(nn.Module):
    ...
    def forward(
        ...
        if self.use_diff:
            ...
            # Compute loss
            assert noise_pred.shape == noise.shape == actions.shape, f"Shape mismatch: {noise_pred.shape} vs {noise.shape} vs {actions.shape}"
    ...
    def predict_action(
        ...
        **kwargs: str
        ...
        def unnormalize_actions(normalized_actions):
            ...
            # derive action_dim dynamically from stats
            dyn_action_dim = len(action_norm_stats["q01"]) if "q01" in action_norm_stats else action_dim

            mask = np.array(mask).reshape(normalized_actions.shape[0], dyn_action_dim)
            action_high = action_high.reshape(normalized_actions.shape[0], dyn_action_dim)
            action_low = action_low.reshape(normalized_actions.shape[0], dyn_action_dim)
            
            normalized_actions = np.clip(normalized_actions, -1, 1)
            
            # remove gripper discretization; UAV actions are continuous
            actions = np.where(
                mask,
    ...
    def predict_action_batch(
        ...
        **kwargs: str
        ...
        action_high, action_low = np.array(action_norm_stats["q99"]), np.array(action_norm_stats["q01"])
        normalized_actions = np.clip(normalized_actions, -1, 1)
        # Remove gripper discretization; UAV actions are continuous 6-DoF
        actions = np.where(
            mask,
            0.5 * (normalized_actions + 1) * (action_high - action_low) + action_low,
            normalized_actions,
